...
#task(label: "b")[
    #enum(
        [Rule 2 eliminates path 3 because of the local preference of 10 and the other paths having not local preference causing a default preference of 100. Remaining routes: 
        #list(
            [`B -> A -> C -> J -> N -> O -> Q -> M`],
            [`B -> C -> J -> N -> O -> Q -> M`],
            [`B -> A -> C -> E -> F -> P -> M`],
